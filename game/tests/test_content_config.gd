...
func _initialize() -> void:
	...
	check(original.stories.get("opening", []).size() == 4 and original.stories.get("mode1_opening", []).size() == 3 and original.ui.size() == 42, "legacy and Mode 1 stories plus all UI controls parsed")
	if original.loaded:
