...
func _initialize() -> void:
	...
	assert(sim.team_xp == 54 and sim.team_level == 1)
	sim.grant_xp(sim.crystal.next_threshold(sim.run_state) - sim.team_xp)
	assert(sim.team_level == 2)
	...
	sim.grant_xp(200)
	assert(sim.run_state.pending_level_ups >= 1 and sim.state == "running")
	sim.tick(14.1)
	assert(sim.state == "reward")
