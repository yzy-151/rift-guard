...
func check(ok: bool, message: String) -> void:
	...
	if not ok:
		failures += 1
	print(("M5 PASS: " if ok else "M5 FAIL: ") + message)
...
func capture(main, name: String) -> void:
	...
	var result: int = main.get_viewport().get_texture().get_image().save_png("user://m5-" + name + ".png")
...
func run(main) -> void:
	...
	await main.get_tree().process_frame
	await main.get_tree().process_frame
	check(main.content.loaded, "external workbook is active")
	...
	var original_opening: Array = main.story.stories.opening.duplicate(true)
	main.story.stories.opening = []
	main.test_mode = false
	main.primary()
	check(main.sim.state == "running" and not main.story.active, "empty opening starts combat without locking")
	main.story.stories.opening = original_opening
	main.restart()
	main.test_mode = true
	await key(KEY_F6)
	check(main.picker.visible, "F6 opens story preview selector")
	var preview_elapsed: float = main.sim.elapsed
	main._physics_process(1.0)
	check(main.sim.elapsed == preview_elapsed, "preview selector freezes simulation")
	main.picker.list.select(0)
	main.picker.play()
	check(main.previewing and main.story.active, "selected scene previews")
	main.dialogue.skip()
	check(not main.previewing and main.sim.state == "ready" and main.story.seen.is_empty(), "preview preserves actual run story progress")
	await key(KEY_F7)
	check(main.fx.active.size() == 2, "F7 starts both supplied sprite sheet effects")
	main.fx.advance(0.35)
	await capture(main, "effects")
	main.effect_preview = 0
	main.fx.active.clear()
	main.hud.signature = ""
	main.refresh()
	var region_ok: bool = true
	for kind in ["heal", "vaporize"]:
		for index in (10 if kind == "heal" else 6):
			var rect: Rect2 = main.fx.frame_region(kind, index)
			region_ok = region_ok and rect.position.x >= 0 and rect.position.y >= 0 and rect.end.x <= 2048 and rect.end.y <= (1152 if kind == "heal" else 1158)
	check(region_ok, "all effect frame rectangles remain inside source atlas")
	main.content.assets["qa_portrait"] = "res://assets/portraits/saria.png"
	main.dialogue.set_picture(main.dialogue.portrait, "qa_portrait", Vector2(400, 120), Vector2(520, 665), 0.75, true)
	check(main.dialogue.portrait.position == Vector2(400, 120) and main.dialogue.portrait.size == Vector2(390, 498.75) and main.dialogue.portrait.flip_h, "portrait position scale and flip apply")
	main.dialogue.set_picture(main.dialogue.partner, "", Vector2.ZERO, Vector2(490, 650), 1.0, false)
	check(not main.dialogue.partner.visible, "empty partner hides image")
	main.content.assets.erase("qa_portrait")
	main.content.textures.erase("qa_portrait")
	...
	for attempt in 3:
		...
		main.dialogue.skip()
	print("M5 UI TESTS: %d checks; %d failures" % [checks, failures])
