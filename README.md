...
V26 在统一动画状态机、战斗时间轴、透明特效导入契约、可复用特效池、敌人/弹道/特效数量预算、过载时的视觉降级、存档迁移、跨数据引用校验和结构化错误日志之上加入九张地图的战术契约、31 个阶段招式槽位（29 个独立动作）与匹配判定。八个战役关卡、十二名角色和六个 Boss 都由 JSON 配置驱动。

## 运行与验收

Windows 构建位于 `builds/rift-guard-v26/RiftGuard-V26.exe`。Godot 4.4 工程入口为 `game/project.godot`。

```powershell
Godot_v4.4.1-stable_win64_console.exe --headless --path game -- --v26-test
```

V26 自动验收包含 51 项专项检查，并继续运行 V25 的 93 项和 V20 的 46 项回归检查。V26 专项覆盖战术契约、奖励、31 个 Boss 阶段招式槽位（29 个独立动作）、九类几何预警、素材就绪度与技能取消交互。当前 README 图片展示 V20 的基础界面，V26 图形截图需要在有桌面图形上下文的会话中重新生成。
