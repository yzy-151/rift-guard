...
master，[V14 Windows试玩版](builds/rift-guard-v14/RiftGuard-V14.exe) 已修复普通模式无法点击进入关卡的问题，并加入五类战术地形、三项队形指令、三条波次时间轴、敌人类型预告与强化打击反馈。

master，剧情模式可从主菜单点击“裂隙守望”，再点击第一关并确认编队进入；战斗中按 `Z` 进入全队移动、按 `X` 跟随旅行者、按 `C` 原地坚守，右键下达移动命令。

master，[V14视觉验收](docs/v14-validation/REPORT.md) 包含地图、路线、敌人轮廓与队形HUD截图，V14新增26项、V10—V13与第一期117项均通过。
